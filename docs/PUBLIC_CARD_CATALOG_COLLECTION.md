...
- 총 레코드: 1,591종
- 중복 제거 공개 레코드: 1,588종
- `official_detail_verified`: 신용카드 3종
- `official_catalog`: 신용카드 8종
- `candidate_index`: 신용카드 1,121종
- `operational_candidate`: 체크카드 456종
- 공식 목록 시드: KB국민카드 신용 10종, 체크 5종
- 후보 인덱스: CardGorilla 카드 검색 API 기준 신용 1,124종, 체크 452종
- 알려진 중복 후보: 3종(원본 보존, 공개 목록·검증 대기열에서 제외)
- 활성 계산 모델 연결: 신용카드 3종(현대 아멕스 플래티넘, KB WE:SH All+, KB ALL)
- 공식 검증 대기: 신용카드 1,129종, 체크카드는 대기열 제외
...
- `operational_candidate`: 체크카드 운영후보 상태다. 카드 검색에는 남기지만 공식 상세 검증 진행률, 검증 대기열, 혜택 계산과 추천에는 포함하지 않는다.
- `official_detail_verified`: 카드사 공식 상세 페이지 또는 상품설명서 기준으로 주요 혜택, 실적 조건, 연회비, 제휴 브랜드 차이까지 검증한 뒤 사용할 승격 상태다.

## 운영 원칙

1. 공식 상세 검증과 계산 승격은 신용카드만 진행한다.
...
6. 신규 계산 카드는 기본 숨김으로 도입해 기존 사용자의 카드 순서와 표시 상태를 바꾸지 않는다.
7. 체크카드 계산 초안은 저장 데이터 보존용으로만 유지할 수 있지만 활성 카드·설정·추천에서는 제외한다.
