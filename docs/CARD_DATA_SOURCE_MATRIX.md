...
| `kb-nori2-check` | 노리2 체크카드(KB Pay) | KB국민카드 | 13 | 체크카드 운영후보, 계산 미반영, 저장 데이터 보존 | 공개 초기 공식 검증 대기열 제외 |
...
`sourceNote`는 사람이 읽는 기존 메모로 유지하고, 구조화된 `source`는 모든 카드에 필수로 둡니다. `officialEntryUrl`은 상품별 공식 URL을 아직 안정적으로 잡지 못했을 때 카드사 공식 진입점을 남기는 보조 필드이며, `recheckBatch`는 같은 카드사/월 단위 검증 작업을 추적하는 값입니다. `networks`는 제휴 브랜드 차이가 계산이나 안내에 영향을 주는 카드부터 선택 필드로 추가합니다. 공식 검증이 끝난 현대 아멕스 플래티넘과 KB 신용카드 2종은 `official_verified`, 노리2 체크카드를 포함한 나머지는 `needs_official_recheck` 상태를 유지합니다. 체크카드는 공개 초기 운영후보로만 보존하고 공식 검증·계산 승격 대기열에서는 제외합니다.
