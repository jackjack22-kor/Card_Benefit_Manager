...
### 추천 정확도 · 입력 개선
- **결제추천 조건 미충족 힌트**: 조건 때문에 혜택가치가 0으로 밀린 카드에 사유를 앰버 힌트로 표시(예: `최소 12,000원 이상 필요`, `전월실적 미달`). `scoreCard()`가 `conditions` 배열을 반환하고, `renderRankItem()`이 value=0 & 직접매칭 카드에 `.rank-hint`를 렌더링. (src/lib/recommend.js, src/main.js)
