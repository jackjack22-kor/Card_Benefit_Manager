...
function renderHeader() {
  const shortfalls = getShortfallCards(state);
  ...
  return `
    <header class="hero">
      <div>
        ...
        <h1>카드 혜택 매니저</h1>
        <p>실적·한도·횟수·추천 카드를 한 화면에서 관리합니다.</p>
      </div>
      <div class="hero-summary">
        <div class="summary-pill ${top ? 'warn' : 'good'}">
          <span>${top ? '이번달 우선 카드' : '이번달 상태'}</span>
          <strong>${top ? escapeHtml(top.card.shortName) : '주요 실적 안정'}</strong>
          <small>${top ? `부족 ${won(top.monthlyShortfall || top.annualShortfall)}` : '상세 카드별 확인 가능'}</small>
        </div>
        <button class="icon-button" data-action="toggle-dark">${state.settings.darkMode ? '☀️' : '🌙'}</button>
      </div>
    </header>
  `;
}
  ...
  return `
    ...
    <section class="section-head">
      <div>
        ...
        <h2>카드별 실적 관제판</h2>
        <p>전월실적은 간단히 표시하고, 이번달/연간 실적과 핵심 혜택 사용 상태를 카드별로 보여줍니다.</p>
      </div>
      <button class="ghost" data-action="edit-order">정렬 편집</button>
    </section>

    <section class="shortfall-strip">
      <h3>부족 실적 카드</h3>
      ${shortfalls.length ? shortfalls.slice(0, 5).map(({ card, monthlyShortfall, annualShortfall }) => `
        <button class="shortfall-card" data-open-card="${card.id}">
          <strong>${escapeHtml(card.shortName)}</strong>
          <span>${monthlyShortfall ? `월 ${won(monthlyShortfall)} 부족` : `연 ${won(annualShortfall)} 부족`}</span>
        </button>`).join('') : '<div class="empty-note">현재 입력값 기준으로 부족 실적 카드가 없습니다.</div>'}
    </section>
...
function renderDashboardCard(card) {
  ...
  const annualTarget = Number(override.annualTarget || 0);
  const annualSpend = Number(override.annualSpend || 0);
  ...
  const annualPct = annualTarget ? clamp(annualSpend / annualTarget, 0, 1) : 0;
  const cycle = getCycle(card, override);
  const coreBenefits = card.benefits.filter((benefit) => benefit.priority === 'core').slice(0, 5);
  const prevLabel = override.prevMonthStatus === 'met' ? '전월실적 충족' : override.prevMonthStatus === 'unmet' ? '전월실적 미달' : '전월실적 확인 필요';
  return `
    <article class="card-section theme-${card.theme}" draggable="true" data-card-id="${card.id}">
      <div class="card-top" data-open-card="${card.id}">
        <div>
      ...
      <div class="progress-block">
        <div class="progress-row">
          <div><strong>이번달 실적</strong><span>${monthlyTarget ? `${won(monthlySpend)} / ${won(monthlyTarget)}` : '관리 대상 없음'}</span></div>
          <b class="${monthlyShortfall ? 'warn-text' : 'good-text'}">${monthlyTarget ? (monthlyShortfall ? `${won(monthlyShortfall)} 부족` : '달성') : '-'}</b>
        </div>
        <div class="bar"><i style="width:${monthlyPct * 100}%"></i></div>
      </div>
      <div class="progress-block compact">
        <div class="progress-row">
          <div><strong>연간 실적</strong><span>${annualTarget ? `${won(annualSpend)} / ${won(annualTarget)}` : '관리 대상 없음'}</span></div>
          <b class="${annualShortfall ? 'warn-text' : 'good-text'}">${annualTarget ? (annualShortfall ? `${won(annualShortfall)} 부족` : '달성') : '-'}</b>
        </div>
        ${annualTarget ? `<div class="bar annual"><i style="width:${annualPct * 100}%"></i></div>` : ''}
      </div>
      <div class="home-meta">
        <span>연간 주기: ${escapeHtml(CYCLE_LABELS[cycle.type] || '기본')} · ${escapeHtml(cycle.label)}</span>
      </div>
      <div class="benefit-chips">
        ${coreBenefits.map((benefit) => `<span>${escapeHtml(getBenefitHomeStatus(state, card, benefit))}</span>`).join('')}
      </div>
      <div class="tag-row">
        ${(card.tags || []).slice(0, 6).map((tag) => `<span>${escapeHtml(tag)}</span>`).join('')}
      </div>
      <div class="card-actions">
        <button data-open-card="${card.id}">상세</button>
        <button data-move-up="${card.id}">↑</button>
        <button data-move-down="${card.id}">↓</button>
      </div>
    </article>
  `;
}
...
function renderRecommend() {
  ...
  const valueResults = recommendCards(state, category, amount);
  const fillResults = getFillSpendRecommendations(state, category, amount);
  return `
    <section class="recommend-hero">
      <span class="eyebrow">BEST PICK</span>
      <h2>어떤 카드로 결제할까?</h2>
      <p>업종과 금액을 선택하면 최대 효율 카드와 실적 채우기 후보를 함께 보여줍니다.</p>
      <div class="recommend-inputs">
        <label>결제 예정 금액 <input type="number" min="0" step="1000" value="${amount}" data-field="recommendationAmount"></label>
      </div>
    ...
    <section class="recommend-columns">
      <div>
        <div class="list-head">
          ...
          <h3>최대 혜택 추천</h3>
          <p>${escapeHtml(CATEGORY_MAP[category]?.label || '카테고리')} 결제에서 남은 한도와 포인트 가치를 반영한 순위입니다.</p>
        </div>
        <div class="rank-list">
          ${valueResults.slice(0, 8).map((result, index) => renderRankItem(result, index, 'value')).join('') || '<div class="empty-note">해당 카테고리 추천 결과가 없습니다.</div>'}
        </div>
      </div>
      <aside>
        <div class="list-head">
          ...
          <h3>실적 채우기 추천</h3>
          <p>이번달·연간 부족 실적을 줄이는 데 도움이 되는 카드입니다.</p>
        </div>
        <div class="rank-list">
        ...
        <div class="tip-box">
          <strong>운영 팁</strong>
          <p>추천 결과는 카드사 업종분류, 간편결제 승인명, 입점매장 여부에 따라 실제와 달라질 수 있습니다. 결제 전 카드 상세의 대상/제외 조건을 확인하세요.</p>
        </div>
      </aside>
    </section>
  `;
...
function renderRankItem(result, index, mode = 'value') {
  ...
  const benefit = result.matching[0]?.benefit;
  const caution = benefit?.exclusions || '업종 분류와 결제 경로에 따라 적용 여부 확인 필요';
  ...
  return `
    <article class="rank-item" data-open-card="${result.card.id}">
      ...
      <div class="rank-body">
        ...
        <span>${escapeHtml(benefit?.name || fillText || result.reason)}</span>
        <small class="${result.status.ok ? 'good-text' : result.status.ok === false ? 'bad-text' : 'muted'}">${result.status.ok ? '✓' : result.status.ok === false ? '×' : '•'} ${escapeHtml(result.status.text)}</small>
        <p>${escapeHtml(result.reason)}</p>
        <em>주의: ${escapeHtml(caution)}</em>
      </div>
...
function renderCardDetail() {
  ...
  const cycle = getCycle(selected, override);
  return `
    <section class="detail-layout">
      ...
      <div class="detail-main">
        ...
        <section class="benefit-list">
          <div class="section-head small"><h3>혜택 상세 및 사용 관리</h3><p>월 사용 체크/금액/횟수 입력 시 홈 화면과 연간 카운트에 자동 반영됩니다.</p></div>
...
function renderCardControls(card, override, cycle) {
  return `
    <div class="control-grid">
      <label>전월실적 상태
        <select data-card-field="prevMonthStatus" data-card-id="${card.id}">
          ${option('met', '충족으로 간주', override.prevMonthStatus)}
          ${option('unmet', '미충족', override.prevMonthStatus)}
          ${option('manual', '직접확인', override.prevMonthStatus)}
        </select>
      </label>
      <label>이번달 실적 목표
      ...
      <label>이번달 사용액
        <input type="number" data-card-field="currentMonthSpend" data-card-id="${card.id}" value="${Number(override.currentMonthSpend || 0)}" step="10000">
      </label>
      <label>연간 실적 목표
      ...
      <label>연간 사용액
        <input type="number" data-card-field="annualSpend" data-card-id="${card.id}" value="${Number(override.annualSpend || 0)}" step="100000">
      </label>
      <label>연간 기준
...
function renderBenefitEditor(card, benefit) {
  ...
  const annualCount = getAnnualUsageCount(state, card, benefit);
  const annualValue = getAnnualBenefitValue(state, card, benefit);
  const override = getCardOverride(state, card.id);
  ...
  const benefitCycle = getCycle(card, override, benefit);
  ...
  return `
    <section class="settings-grid">
      <div class="settings-card full-span">
        <h3>단일 HTML 로컬 앱</h3>
        <p>현재 버전은 <code>dist/card-benefit-manager.html</code> 한 파일을 스마트폰에 옮겨 쓰는 방식입니다. 사용 데이터는 이 파일 안이 아니라, 현재 브라우저의 localStorage에 저장됩니다.</p>
        <p>브라우저 데이터 삭제, 다른 브라우저 사용, 스마트폰 변경 시 데이터가 보이지 않을 수 있습니다. 이동 전에는 반드시 JSON 백업을 내보내세요.</p>
      </div>
      ...
      <div class="settings-card">
        <h3>카드 순서 편집</h3>
        <p>홈 화면 표시 순서입니다. 드래그 또는 버튼으로 조정할 수 있습니다.</p>
        <div class="order-list">
          ${getOrderedCards(state).map((card) => `<div><span>${escapeHtml(card.shortName)}</span><button data-move-up="${card.id}">↑</button><button data-move-down="${card.id}">↓</button></div>`).join('')}
        </div>
      </div>
      <div class="settings-card">
        <h3>백업/복원</h3>
        <p>개인 데이터는 브라우저 localStorage에만 저장됩니다. 폰 변경, 브라우저 변경, 사이트 데이터 삭제에 대비하려면 JSON 백업이 필요합니다.</p>
      ...
      <div class="settings-card">
        ...
        <p>3. 새 스마트폰에서 <code>card-benefit-manager.html</code> 열기</p>
...
function renderBackupNotice(className = 'backup-notice') {
  ...
  if (!status.shouldWarn && state.selectedTab !== 'settings') return '';
  return `
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-category]').forEach((button) => button.addEventListener('click', () => setState({ selectedCategory: button.dataset.category })));
  ...
  document.querySelectorAll('[data-mode]').forEach((button) => button.addEventListener('click', () => updateSettings({ recommendMode: button.dataset.mode })));
...
function bindDragSort() {
  ...
  document.querySelectorAll('.card-section[draggable="true"]').forEach((el) => {
    ...
    const endManualDrag = (event) => {
      ...
      if (wasActive) {
        const target = document.elementFromPoint(event.clientX, event.clientY)?.closest('.card-section');
    ...
    el.addEventListener('mousedown', startManualDrag);
    el.addEventListener('mousemove', moveManualDrag);
    el.addEventListener('mouseup', endManualDrag);
  });
}
